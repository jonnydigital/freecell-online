...
- Configure cron with `GOOGLE_APPLICATION_CREDENTIALS` or `GOOGLE_OAUTH_ACCESS_TOKEN`, run `npm run analytics:setup-next-actions`, review the latest `docs/analytics/next-action-dimension-setup/` report, then re-run `npm run metrics:google` and `npm run analytics:next-actions` to decide whether Hint, Undo, New Game, or learning links deserve more prominence in the phone next-action strip.
