...
- Configure cron with `GOOGLE_APPLICATION_CREDENTIALS` or `GOOGLE_OAUTH_ACCESS_TOKEN`, then re-run `npm run metrics:google` and `npm run analytics:next-actions` to decide whether Hint, Undo, New Game, or learning links deserve more prominence in the phone next-action strip.
