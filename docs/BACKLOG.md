...
34. **Mobile viewport QA harness for spoke/domain routes** — next 1 AM build target from the 2026-07-11 evening research pass. The 07-11 FreeCell portrait fix proved the same-origin iframe rig catches true-phone layout bugs, but Klondike/Spider spokes are still not measurable from the current live-browser path. Build a lightweight internal harness or script that can measure true 375/390/414px layouts for FreeCell, Klondike, Spider, and shared generic boards without introducing a heavy browser dependency. Acceptance: card count, card width, clipped-card count, horizontal overflow, top/bottom control visibility, and unused vertical-space metrics are emitted for each route; production navigation stays clean; results are easy to paste into the next QA report.
35. **Per-language analytics review before further i18n expansion** — first localized-route GA4 signal appeared 2026-07-11 (French guide 5 views, Portuguese play 3 views), while Spanish/German/Italian were not visible in the GA4 Home top-pages card. Review page-language/locale event data once enough GSC/GA4 signal exists before adding new languages or deeper localized content.
