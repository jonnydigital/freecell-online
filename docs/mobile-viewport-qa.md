...
- Browser runtime diagnostics are captured while each route loads. The Markdown table shows `JS errors` as `runtime exceptions / console+log errors`; runtime exceptions fail the audit, while console and browser log errors are recorded for triage.
...
- Browser diagnostics: runtime exceptions are treated as hard failures because they usually indicate broken client code. Console and browser log errors are listed in the `Browser Diagnostics` section for follow-up, but do not fail the audit by themselves.
