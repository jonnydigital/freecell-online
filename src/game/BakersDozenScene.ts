...
interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: BakersDozenLocation;
}

export class BakersDozenScene extends Phaser.Scene {
  private engine!: BakersDozenEngine;
  ...
  // Portrait split-row layout
  private portraitTopRowCols: number = 7;
  private portraitBottomRowCols: number = 6;

  // Drag — single card only
  private activeDragCards: CardSprite[] = [];
  private activeDragFrom: BakersDozenLocation | null = null;
  ...
  private pendingSettledMove: { from: BakersDozenLocation; to: BakersDozenLocation } | null = null;
  ...
  create(): void {
    ...
    const { tableau } = dealBakersDozenGame(this.gameNumber);
    this.engine = new BakersDozenEngine(this.gameNumber, tableau);
  ...
  private calculateLayout(): void {
    ...
    if (this.isPortrait) {
      // Portrait: 2 rows of columns (7 on top, 6 on bottom)
      // Card width determined by the wider row (7 cols)
      const usableWidth = w * (1 - 2 * SIDE_MARGIN);
      const gapPx = w * GAP;
      const colsInRow = this.portraitTopRowCols;
      const cardWidthFromWidth = Math.floor(((usableWidth - (colsInRow - 1) * gapPx) / colsInRow) * largeCardsScale);
      ...
      this.boardOffsetX = Math.floor((w - (colsInRow * this.cardWidth + (colsInRow - 1) * gapPx)) / 2);
      this.boardOffsetY = topPad;
      this.foundationRowHeight = this.cardHeight;
    } else {
      // Landscape: all 13 columns in a single row
      const usableWidth = w * (1 - 2 * SIDE_MARGIN);
      const gapPx = w * GAP;
  ...
  private getColumnX(col: number): number {
    if (this.isPortrait) {
      ...
      if (col < this.portraitTopRowCols) {
        // Top row: columns 0-6
        return this.boardOffsetX + col * (this.cardWidth + gapPx);
      } else {
        // Bottom row: columns 7-12, centered
        const bottomCol = col - this.portraitTopRowCols;
        const bottomRowWidth = this.portraitBottomRowCols * this.cardWidth + (this.portraitBottomRowCols - 1) * gapPx;
        const bottomOffsetX = Math.floor((this.scale.width - bottomRowWidth) / 2);
        return bottomOffsetX + bottomCol * (this.cardWidth + gapPx);
      }
    }
  ...
  private getCurrentOverlap(): number {
    ...
    // In portrait mode, the bottom row of columns shares vertical space, so use reduced available height
    const effectiveAvailable = this.isPortrait
      ? (availableHeight - this.cardHeight - this.cascadeGap) / 2 + this.cardHeight
      : availableHeight;

    const targetOverlap = maxPileLength > 1
      ? Math.floor((effectiveAvailable * 0.95 - this.cardHeight) / (maxPileLength - 1))
      : this.cardHeight;
    const maxDesiredOverlap = Math.floor(this.cardHeight * MAX_CASCADE_OVERLAP_FRAC);
    const maxFittingOverlap = maxPileLength > 1
      ? Math.floor((effectiveAvailable - this.cardHeight) / (maxPileLength - 1))
      : this.cardHeight;
  ...
  // Foundations: single row above the tableau, centered
  private getFoundationPosition(index: number): { x: number; y: number } {
    const gapPx = this.scale.width * GAP;
    const totalFoundationWidth = NUM_FOUNDATIONS * this.cardWidth + (NUM_FOUNDATIONS - 1) * gapPx;
    const foundationOffsetX = Math.floor((this.scale.width - totalFoundationWidth) / 2);
    return {
      x: foundationOffsetX + index * (this.cardWidth + gapPx),
      y: this.boardOffsetY,
    };
  }
  ...
  private getTableauCardPosition(col: number, row: number): { x: number; y: number } {
    if (this.isPortrait && col >= this.portraitTopRowCols) {
      // Bottom row columns: offset vertically
      const topRowMaxPile = Math.max(
        ...this.engine.getState().tableau.slice(0, this.portraitTopRowCols).map(c => c.length),
        1
      );
      const overlap = this.getCurrentOverlap();
      const topRowHeight = this.cardHeight + (topRowMaxPile - 1) * overlap;
      const bottomRowBaseY = this.getTableauBaseY() + topRowHeight + this.cascadeGap;
      return { x: this.getColumnX(col), y: bottomRowBaseY + row * overlap };
    }
  ...
  private drawSlots(): void {
    ...
    // Foundation slots (single row, centered)
  ...
  private dealCards(staggered: boolean = false): void {
    ...
    // Deal tableau cards (13 columns, no pre-placed aces)
    for (let col = 0; col < NUM_COLS; col++) {
      const colCards = state.tableau[col];
      for (let row = 0; row < colCards.length; row++) {
  ...
  private executeMoveAndAnimate(from: BakersDozenLocation, to: BakersDozenLocation): void {
    try {
    ...
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        for (let i = 0; i < NUM_COLS; i++) {
          ...
          // Try foundation
          const toFoundation: BakersDozenLocation = { type: 'foundation', suit: card.suit };
          if (this.engine.isLegalMove(from, toFoundation)) {
            this.engine.executeMove(from, toFoundation);
            moved = true;
            break;
          }
        }
  ...
  private flashHintCards(hint: BakersDozenMove): void {
  ...
  private getTableauColAtPoint(x: number, y: number): number {
    ...
    for (let col = 0; col < NUM_COLS; col++) {
      ...
      // Get the base Y for this specific column (accounts for portrait split-row)
      let colBaseY: number;
      if (this.isPortrait && col >= this.portraitTopRowCols) {
        const topRowMaxPile = Math.max(
          ...this.engine.getState().tableau.slice(0, this.portraitTopRowCols).map(c => c.length),
        ...
        const topRowHeight = this.cardHeight + (topRowMaxPile - 1) * overlap;
        colBaseY = tableauBaseY + topRowHeight + this.cascadeGap;
      } else {
        colBaseY = tableauBaseY;
      }

      const pileHeight = colCards.length > 0
        ? this.cardHeight + (colCards.length - 1) * overlap
        : this.cardHeight;
      if (y >= colBaseY - 5 && y <= colBaseY + pileHeight + 5) {
        return col;
      }
    }
    return -1;
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    if (foundIdx !== -1) {
      ...
      const to: BakersDozenLocation = { type: 'foundation', suit };
    ...
    const col = this.getTableauColAtPoint(x, y);
    if (col !== -1) {
      const to: BakersDozenLocation = { type: 'tableau', index: col };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    ...
    let tappedFrom: BakersDozenLocation | null = null;
