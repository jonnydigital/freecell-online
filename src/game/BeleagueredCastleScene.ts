...
import { BeleagueredCastleEngine, BeleagueredCastleLocation, BeleagueredCastleMove } from '../engine/BeleagueredCastleEngine';
...
interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: BeleagueredCastleLocation;
}

export class BeleagueredCastleScene extends Phaser.Scene {
  private engine!: BeleagueredCastleEngine;
  ...
  private activeDragFrom: BeleagueredCastleLocation | null = null;
  ...
  private pendingSettledMove: { from: BeleagueredCastleLocation; to: BeleagueredCastleLocation } | null = null;
  ...
  create(): void {
    ...
    const { tableau, aces } = dealBeleagueredCastleGame(this.gameNumber);
    this.engine = new BeleagueredCastleEngine(this.gameNumber, tableau, aces);
  ...
  private calculateLayout(): void {
    ...
    // Layout: 4 foundations centered on top row, 8 tableau columns below
    // Use 8 columns for card width calculation
  ...
  // Top row layout: foundations centered — columns 2,3,4,5
  ...
  private drawSlots(): void {
    ...
    // 4 Foundation slots (columns 2-5, centered)
  ...
  private dealCards(staggered: boolean = false): void {
    ...
    for (let i = 0; i < NUM_FOUNDATIONS; i++) {
      ...
      const ace = pile[0]; // The pre-placed ace
      const pos = this.getFoundationPosition(i);
    ...
    // Then: deal cascade cards (8 columns, 6 cards each)
  ...
  /**
   * Only the top (last) card of a cascade can be picked up.
   */
  ...
  private executeMoveAndAnimate(from: BeleagueredCastleLocation, to: BeleagueredCastleLocation): void {
    try {
  ...
  private autoFinish(): void {
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        // Try moving cards from tableau to foundations
        let moved = false;
        const state = this.engine.getState();
        ...
        for (let i = 0; i < NUM_COLS; i++) {
          ...
          const to: BeleagueredCastleLocation = { type: 'foundation', suit: card.suit };
  ...
  private flashHintCards(hint: BeleagueredCastleMove): void {
  ...
  private tryStartDrag(x: number, y: number): void {
    // No free cells in Beleaguered Castle — only cascade card drag
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    if (fIdx !== -1) {
      ...
      const to: BeleagueredCastleLocation = { type: 'foundation', suit };
    ...
    const col = this.getCascadeColumnAtPoint(x, y);
    if (col !== -1) {
      const to: BeleagueredCastleLocation = { type: 'tableau', index: col };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    ...
    let tappedFrom: BeleagueredCastleLocation | null = null;

    // Check cascades (no free cells in this variant)
    const col = this.getCascadeColumnAtPoint(x, y);
    if (col !== -1) {
      const cascade = state.tableau[col];
      if (cascade.length > 0) {
    ...
    if (tappedCardId && tappedFrom && tappedCardId === this.lastTapCardId && now - this.lastTapTime < 400) {
      ...
      const card = cascade.length > 0 ? cascade[cascade.length - 1] : null;
