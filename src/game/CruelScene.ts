...
import { CruelEngine, CruelLocation, CruelMove } from '../engine/CruelEngine';
...
interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: CruelLocation;
}

export class CruelScene extends Phaser.Scene {
  private engine!: CruelEngine;
  ...
  private activeDragFrom: CruelLocation | null = null;
  ...
  private pendingSettledMove: { from: CruelLocation; to: CruelLocation } | null = null;
  ...
  create(): void {
    ...
    const { tableau, aces } = dealCruelGame(this.gameNumber);
    this.engine = new CruelEngine(this.gameNumber, tableau, aces);
  ...
  private calculateLayout(): void {
    ...
    // Layout: 4 foundations + redeal button on top row, then 2 rows of 6 piles below
    ...
    this.rowGap = Math.floor(h * 0.015);
  ...
  private getCurrentOverlap(): number {
    ...
    // Two rows of piles — each row gets half the remaining height
    const totalAvailable = this.scale.height - topRow - 4;
    const rowHeight = (totalAvailable - this.rowGap) / 2;
  ...
  /** Get the pile index (0-11) row (0 or 1) and column (0-5) */
  ...
  private createRedealButton(): void {
    ...
    bg.fillStyle(hexToInt(this.currentTheme.feltNoiseLight), 0.15);
    bg.fillRoundedRect(0, 0, this.cardWidth, this.cardHeight, 6);
    bg.lineStyle(2, hexToInt(this.currentTheme.feltNoiseLight), 0.6);
    bg.strokeRoundedRect(0, 0, this.cardWidth, this.cardHeight, 6);
    container.add(bg);

    // Redeal icon (↻)
    const iconSize = Math.floor(this.cardWidth * 0.35);
    const icon = this.add.text(this.cardWidth / 2, this.cardHeight * 0.38, '\u21BB', {
    ...
    // Label
    const labelSize = Math.floor(this.cardWidth * 0.16);
    const label = this.add.text(this.cardWidth / 2, this.cardHeight * 0.7, 'REDEAL', {
  ...
  private dealCards(staggered: boolean = false): void {
    ...
    // First: animate aces flying to foundations
    for (let i = 0; i < NUM_FOUNDATIONS; i++) {
      const suit = this.FOUNDATION_SUITS[i];
      const pile = state.foundations.get(suit)!;
  ...
  private executeMoveAndAnimate(from: CruelLocation, to: CruelLocation): void {
    try {
    ...
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        for (let i = 0; i < NUM_PILES; i++) {
          ...
          const to: CruelLocation = { type: 'foundation', suit: card.suit };
  ...
  private flashHintCards(hint: CruelMove): void {
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    if (fIdx !== -1) {
      ...
      const to: CruelLocation = { type: 'foundation', suit };
    ...
    const pile = this.getPileAtPoint(x, y);
    if (pile !== -1) {
      const to: CruelLocation = { type: 'tableau', index: pile };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    ...
    let tappedFrom: CruelLocation | null = null;
