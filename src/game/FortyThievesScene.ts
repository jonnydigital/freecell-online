...
import { FortyThievesEngine, FortyThievesLocation, FortyThievesMove } from '../engine/FortyThievesEngine';
...
const MIN_CASCADE_OVERLAP_PX = 20;    // Slightly smaller for 10 columns
const MAX_CASCADE_OVERLAP_FRAC = 0.75;
const SIDE_MARGIN = 0.015;            // Tighter margins for 10 cols
const GAP = 0.01;                      // Tighter gap for 10 cols
const NUM_COLS = 10;
const NUM_FOUNDATIONS = 8;

interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: FortyThievesLocation;
}

export class FortyThievesScene extends Phaser.Scene {
  private engine!: FortyThievesEngine;
  ...
  // Drag — single card only for Forty Thieves
  private activeDragCards: CardSprite[] = [];
  private activeDragFrom: FortyThievesLocation | null = null;
  ...
  private pendingSettledMove: { from: FortyThievesLocation; to: FortyThievesLocation } | null = null;
  ...
  // Foundation key order: 2 piles per suit
  // S-0, S-1, H-0, H-1, D-0, D-1, C-0, C-1
  private readonly FOUNDATION_KEYS: string[] = [
    'S-0', 'S-1', 'H-0', 'H-1', 'D-0', 'D-1', 'C-0', 'C-1'
  ];

  constructor() {
    super({ key: 'FortyThievesScene' });
  }
  ...
  create(): void {
    ...
    const { tableau, stock } = dealFortyThievesGame(this.gameNumber);
    this.engine = new FortyThievesEngine(this.gameNumber, tableau, stock);
  ...
  private calculateLayout(): void {
    ...
    // 10 columns for Forty Thieves — this is the width constraint
  ...
  private getCascadeCardY(_col: number, row: number): number {
    // All cards are face-up in Forty Thieves, so uniform overlap
  ...
  private getFoundationPosition(index: number): { x: number; y: number } {
    // 8 foundations at columns 2-9 in the top row
    return { x: this.getColumnX(2 + index), y: this.boardOffsetY };
  }
  ...
  private repositionAllCards(): void {
    ...
    // Waste cards (show top card only — Forty Thieves draws 1 at a time)
    const wastePos = this.getWastePosition();
    ...
    // Foundation cards (8 foundations)
    for (let i = 0; i < NUM_FOUNDATIONS; i++) {
      const key = this.FOUNDATION_KEYS[i];
      const pile = state.foundations.get(key)!;
      const pos = this.getFoundationPosition(i);
      for (let j = 0; j < pile.length; j++) {
        const sprite = this.cardSprites.get(pile[j].id);
        if (sprite) {
          sprite.x = pos.x; sprite.y = pos.y;
          sprite.sourceLocation = { type: 'foundation', key };
  ...
  private dealCards(staggered: boolean = false): void {
    ...
    // Deal cascade cards (10 columns, 4 cards each)
    for (let col = 0; col < NUM_COLS; col++) {
      const cascade = state.tableau[col];
      for (let row = 0; row < cascade.length; row++) {
  ...
  /**
   * Forty Thieves: only the top (last) card of a cascade can be picked up.
   * Returns the index of the top card if the point hits it, -1 otherwise.
   */
  private getCascadePickupIndexAtPoint(col: number, x: number, y: number): number {
    ...
    // Only the top card is pickable in Forty Thieves
    const topIdx = cascade.length - 1;
    const topCard = cascade[topIdx];
  ...
  private executeMoveAndAnimate(from: FortyThievesLocation, to: FortyThievesLocation): void {
    try {
      ...
      // Rebuild sprites to show state changes
      this.time.delayedCall(50, () => {
        ...
        // Check win (104 cards on foundations)
  ...
  private winCelebration(): void {
    ...
    // Collect cards from all 8 foundations (top card first, then next, etc.)
    const state = this.engine.getState();
    const cardQueue: { card: Card; foundationIdx: number }[] = [];
    for (let rank = 13; rank >= 1; rank--) {
      for (let fi = 0; fi < NUM_FOUNDATIONS; fi++) {
        const key = this.FOUNDATION_KEYS[fi];
        const pile = state.foundations.get(key);
    ...
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        if (state.waste.length > 0) {
          ...
          const from: FortyThievesLocation = { type: 'waste' };
          const to: FortyThievesLocation = { type: 'foundation', key: this.findFoundationKeyForCard(card) };
          if (to.key && this.engine.isLegalMove(from, to)) {
        ...
        if (!moved) {
          for (let i = 0; i < NUM_COLS; i++) {
            ...
            const from: FortyThievesLocation = { type: 'tableau', index: i };
            const foundKey = this.findFoundationKeyForCard(card);
            if (!foundKey) continue;
            const to: FortyThievesLocation = { type: 'foundation', key: foundKey };
  ...
  /**
   * Find which foundation key a card can go to.
   * Each suit has 2 foundations (e.g., 'S-0' and 'S-1').
   * Returns the first foundation key that accepts this card, or undefined.
   */
  private findFoundationKeyForCard(card: Card): string | undefined {
    const state = this.engine.getState();
    const suitChar = card.suit as string;
    for (let idx = 0; idx < 2; idx++) {
      const key = `${suitChar}-${idx}`;
      const pile = state.foundations.get(key);
      if (!pile) continue;
      if (pile.length === 0 && card.rank === 1) return key;
      if (pile.length > 0 && pile[pile.length - 1].rank === card.rank - 1 && pile[pile.length - 1].suit === card.suit) return key;
    }
    return undefined;
  }

  private flashHintCards(hint: FortyThievesMove): void {
  ...
  update(_time: number, delta: number): void {
    ...
    if (this.isDragging && this.activeDragTarget) {
      ...
      // Single card drag only in Forty Thieves
  ...
  private tryStartDrag(x: number, y: number): void {
    // Check stock click — draws 1 card, no recycling
    ...
    // Check waste card drag (single card)
    ...
    // Check cascade card drag — SINGLE card only
    ...
    if (col !== -1) {
      ...
      // Only pick up the single top card
      const sprite = this.cardSprites.get(cascade[cardIdx].id);
      if (sprite) {
        this.activeDragCards = [sprite];
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    // Check foundation drop (single card — always single in Forty Thieves)
    const fIdx = this.getFoundationIndexAtPoint(x, y);
    if (fIdx !== -1) {
      const foundationKey = this.FOUNDATION_KEYS[fIdx];
      const to: FortyThievesLocation = { type: 'foundation', key: foundationKey };
    ...
    const col = this.getCascadeColumnAtPoint(x, y);
    if (col !== -1) {
      const to: FortyThievesLocation = { type: 'tableau', index: col };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
        const tgtRow = this.engine.getState().tableau[col].length;
        this.settleTargets = [this.getCascadeCardPosition(col, tgtRow)];
        this.lastDragPointerPosition = null;
        this.animateDraggedCardsToTargets(this.settleTargets, () => {
          this.isSettlingDrag = false;
          if (this.pendingSettledMove) {
    ...
    if (this.activeDragFrom.type === 'tableau') {
      ...
      // Snap back to where the top card sits (which is still the last position since we haven't moved it yet)
      const snapRow = Math.max(0, fromCascade.length - 1);
      this.settleTargets = [this.getCascadeCardPosition(tableauFrom.index, snapRow)];
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    ...
    let tappedFrom: FortyThievesLocation | null = null;
    ...
    if (tappedCardId && tappedFrom && tappedCardId === this.lastTapCardId && now - this.lastTapTime < 400) {
      ...
      // Find an appropriate foundation key for this card
      const card = tappedFrom.type === 'waste'
        ? state.waste[state.waste.length - 1]
        : state.tableau[(tappedFrom as { type: 'tableau'; index: number }).index][
            state.tableau[(tappedFrom as { type: 'tableau'; index: number }).index].length - 1
          ];
      const foundKey = this.findFoundationKeyForCard(card);
      if (foundKey) {
        const to: FortyThievesLocation = { type: 'foundation', key: foundKey };
