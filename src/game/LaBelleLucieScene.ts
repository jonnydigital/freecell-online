...
import { LaBelleLucieEngine, LaBelleLucieLocation, LaBelleLucieMove } from '../engine/LaBelleLucieEngine';
...
const MIN_CASCADE_OVERLAP_PX = 18;
const MAX_CASCADE_OVERLAP_FRAC = 0.5;
...
interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: LaBelleLucieLocation;
}

export class LaBelleLucieScene extends Phaser.Scene {
  private engine!: LaBelleLucieEngine;
  ...
  private activeDragFrom: LaBelleLucieLocation | null = null;
  ...
  private pendingSettledMove: { from: LaBelleLucieLocation; to: LaBelleLucieLocation } | null = null;
  ...
  // Redeal button
  private redealButton: Phaser.GameObjects.Container | null = null;

  // Merci mode
  private isMerciMode: boolean = false;
  private merciHighlights: Phaser.GameObjects.GameObject[] = [];
  private merciSelectedCard: { pileIndex: number; cardIndex: number; card: Card } | null = null;

  // Foundation suit order
  private readonly FOUNDATION_SUITS: Suit[] = [Suit.Spades, Suit.Hearts, Suit.Diamonds, Suit.Clubs];

  constructor() {
    super({ key: 'LaBelleLucieScene' });
  }
  ...
  create(): void {
    ...
    const { tableau } = dealLaBelleLucieGame(this.gameNumber);
    this.engine = new LaBelleLucieEngine(this.gameNumber, tableau);
  ...
  private getNumPiles(): number {
    return this.engine.getState().tableau.length;
  }

  private getNumRows(): number {
    return Math.ceil(this.getNumPiles() / NUM_COLS);
  }
  ...
  private calculateLayout(): void {
    ...
    this.rowGap = Math.floor(h * 0.012);

    const numRows = 3; // 3 rows of fans
    const vertBudget = h - topPad - this.cascadeGap - 4;
    const maxCardHeight = Math.floor((vertBudget - 6 * MIN_CASCADE_OVERLAP_PX) / (numRows + 1));
  ...
  private getCurrentOverlap(): number {
    ...
    const totalAvailable = this.scale.height - topRow - 4;
    const rowHeight = (totalAvailable - (numRows - 1) * this.rowGap) / numRows;

    const targetOverlap = maxPileLength > 1
      ? Math.floor((rowHeight * 0.95 - this.cardHeight) / (maxPileLength - 1))
      : this.cardHeight;
    const maxDesiredOverlap = Math.floor(this.cardHeight * MAX_CASCADE_OVERLAP_FRAC);
    const maxFittingOverlap = maxPileLength > 1
      ? Math.floor((rowHeight - this.cardHeight) / (maxPileLength - 1))
      : this.cardHeight;
  ...
  private getPileRowCol(pileIndex: number): { row: number; col: number } {
    return { row: Math.floor(pileIndex / NUM_COLS), col: pileIndex % NUM_COLS };
  }

  private getPileBaseY(pileRow: number): number {
    const topRow = this.boardOffsetY + this.topRowHeight + this.cascadeGap;
    const numRows = this.getNumRows();
    const totalAvailable = this.scale.height - topRow - 4;
    const rowHeight = (totalAvailable - (numRows - 1) * this.rowGap) / numRows;
    return topRow + pileRow * (rowHeight + this.rowGap);
  }

  private getPileCardY(pileIndex: number, cardRow: number): number {
    const { row } = this.getPileRowCol(pileIndex);
    const baseY = this.getPileBaseY(row);
    const overlap = this.getCurrentOverlap();
    return baseY + cardRow * overlap;
  }

  private getPileCardPosition(pileIndex: number, cardRow: number): { x: number; y: number } {
    const { col } = this.getPileRowCol(pileIndex);
    return { x: this.getColumnX(col), y: this.getPileCardY(pileIndex, cardRow) };
  }

  // Top row layout: foundations on left (cols 0-3), redeal button on right (col 5)
  private getFoundationPosition(index: number): { x: number; y: number } {
    return { x: this.getColumnX(index), y: this.boardOffsetY };
  }

  private getRedealButtonPosition(): { x: number; y: number } {
    return { x: this.getColumnX(5), y: this.boardOffsetY };
  }

  // ===== RENDERING =====

  private createBoard(): void { this.drawSlots(); this.createRedealButton(); }
  private rebuildBoard(): void { this.drawSlots(); this.createRedealButton(); }

  private drawSlots(): void {
    this.slotGraphics.forEach(g => g.destroy());
    this.slotGraphics = [];

    // 4 Foundation slots (columns 0-3)
  ...
  private createRedealButton(): void {
    if (this.redealButton) {
      this.redealButton.destroy();
      this.redealButton = null;
    }

    const pos = this.getRedealButtonPosition();
    const container = this.add.container(pos.x, pos.y);
    container.setSize(this.cardWidth, this.cardHeight);
    container.setDepth(5);

    const canRedeal = this.engine.canRedeal();
    const isMerci = this.engine.isMerciAvailable();
    const bgAlpha = (canRedeal || isMerci) ? 0.15 : 0.06;

    // Background
    const bg = this.add.graphics();
    bg.fillStyle(hexToInt(this.currentTheme.feltNoiseLight), bgAlpha);
    bg.fillRoundedRect(0, 0, this.cardWidth, this.cardHeight, 6);
    bg.lineStyle(2, hexToInt(this.currentTheme.feltNoiseLight), canRedeal || isMerci ? 0.6 : 0.2);
    bg.strokeRoundedRect(0, 0, this.cardWidth, this.cardHeight, 6);
    container.add(bg);

    // Icon and label
    const iconSize = Math.floor(this.cardWidth * 0.3);
    const labelSize = Math.floor(this.cardWidth * 0.13);
    const textAlpha = (canRedeal || isMerci) ? 1.0 : 0.3;

    if (isMerci && !this.engine.getState().merciUsed) {
      // Merci button
      const icon = this.add.text(this.cardWidth / 2, this.cardHeight * 0.32, '\u2764', {
        fontSize: `${iconSize}px`,
        color: '#ff6666',
        fontFamily: 'serif',
      });
      icon.setOrigin(0.5);
      icon.setAlpha(textAlpha);
      container.add(icon);

      const label = this.add.text(this.cardWidth / 2, this.cardHeight * 0.6, 'MERCI', {
      ...
      label.setOrigin(0.5);
      label.setAlpha(textAlpha);
      container.add(label);

      const subLabel = this.add.text(this.cardWidth / 2, this.cardHeight * 0.78, 'Draw 1 card', {
        fontSize: `${Math.floor(labelSize * 0.75)}px`,
        color: this.currentTheme.feltNoiseLight,
        fontFamily: 'Inter, sans-serif',
      });
      subLabel.setOrigin(0.5);
      subLabel.setAlpha(0.5);
      ...
      const remainingRedeals = 2 - this.engine.getRedealCount();
      const label = this.add.text(this.cardWidth / 2, this.cardHeight * 0.6, 'REDEAL', {
        fontSize: `${labelSize}px`,
        color: this.currentTheme.feltNoiseLight,
        fontFamily: 'Inter, sans-serif',
        fontStyle: 'bold',
      ...
      const countLabel = this.add.text(this.cardWidth / 2, this.cardHeight * 0.78, `${remainingRedeals} left`, {
        fontSize: `${Math.floor(labelSize * 0.75)}px`,
        color: this.currentTheme.feltNoiseLight,
        fontFamily: 'Inter, sans-serif',
      });
      countLabel.setOrigin(0.5);
      countLabel.setAlpha(0.5);
      container.add(countLabel);
    }

    // Make interactive
    const hitArea = this.add.zone(pos.x + this.cardWidth / 2, pos.y + this.cardHeight / 2, this.cardWidth, this.cardHeight);
    hitArea.setInteractive({ useHandCursor: true });
    hitArea.setDepth(6);
    hitArea.on('pointerdown', () => {
      if (isMerci && !this.engine.getState().merciUsed) {
        this.toggleMerciMode();
      } else if (canRedeal) {
        this.handleRedeal();
      }
    });
    this.slotGraphics.push(hitArea);

    this.redealButton = container;
  ...
  private repositionAllCards(): void {
    const state = this.engine.getState();
    const numPiles = state.tableau.length;

    // Pile cards
    for (let pile = 0; pile < numPiles; pile++) {
      const pileCards = state.tableau[pile];
      for (let row = 0; row < pileCards.length; row++) {
        const sprite = this.cardSprites.get(pileCards[row].id);
        if (sprite) {
          const pos = this.getPileCardPosition(pile, row);
          sprite.x = pos.x; sprite.y = pos.y;
          sprite.sourceLocation = { type: 'tableau', index: pile };
          sprite.setDepth(pile * 10 + row + 10);
        }
      }
    }
  ...
  private getPilePickupIndexAtPoint(pile: number, x: number, y: number): number {
    const pileCards = this.engine.getState().tableau[pile];
    if (pileCards.length === 0) return -1;

    const topIdx = pileCards.length - 1;
    const topCard = pileCards[topIdx];
  ...
  private enterMerciMode(): void {
    if (!this.engine.isMerciAvailable()) return;
    this.isMerciMode = true;
    this.merciSelectedCard = null;

    // Highlight all cards that have valid merci destinations
    const state = this.engine.getState();
    for (let pileIdx = 0; pileIdx < state.tableau.length; pileIdx++) {
      ...
      for (let cardIdx = 0; cardIdx < pile.length; cardIdx++) {
        ...
        const destinations = this.engine.getMerciDestinations(card);
        if (destinations.length > 0) {
          ...
          if (sprite) {
            ...
            glow.strokeRoundedRect(sprite.x - 2, sprite.y - 2, this.cardWidth + 4, this.cardHeight + 4, 8);
            glow.setDepth(sprite.depth + 0.5);
            this.merciHighlights.push(glow);
  ...
  private exitMerciMode(): void {
    this.isMerciMode = false;
    this.merciSelectedCard = null;
    for (const obj of this.merciHighlights) {
      this.tweens.killTweensOf(obj);
      obj.destroy();
    }
    this.merciHighlights = [];
  }

  private handleMerciCardClick(x: number, y: number): boolean {
    if (!this.isMerciMode) return false;
    ...
    for (let pileIdx = 0; pileIdx < state.tableau.length; pileIdx++) {
      const pile = state.tableau[pileIdx];
      // Check cards in reverse (top first)
      for (let cardIdx = pile.length - 1; cardIdx >= 0; cardIdx--) {
        ...
        if (this.isPointWithinCardBounds(x, y, sprite.x, sprite.y, this.cardWidth, this.cardHeight)) {
          ...
          // Auto-pick best destination (foundation first, then tableau)
          const dest = destinations[0];
          const move = this.engine.executeMerciMove(pileIdx, cardIdx, dest);
          if (move) {
            soundManager.cardPlace();
            if (dest.type === 'foundation') {
              soundManager.cardToFoundation(card.rank);
            }
            this.exitMerciMode();
            this.invalidateOverlapCache();
            this.recreateAllCardSprites();
            this.repositionAllCards();
            this.createRedealButton(); // update button state
    ...
    // Clicked outside any card — exit merci mode
    this.exitMerciMode();
    return true;
  }
  ...
  private handleRedeal(): void {
    if (this.isDealAnimating || this.isDragging) return;
    if (!this.engine.canRedeal()) return;

    const totalCards = this.engine.getState().tableau.reduce((sum, pile) => sum + pile.length, 0);
    if (totalCards === 0) return;

    const move = this.engine.redeal();
    if (!move) return;

    soundManager.cardPlace();
    this.isDealAnimating = true;

    // Animate: sweep cards up then re-deal
    const w = this.scale.width;
    const centerX = w / 2 - this.cardWidth / 2;
    const centerY = -this.cardHeight;

    // Phase 1: Sweep all cards to center top
    const allSprites = Array.from(this.cardSprites.values()).filter(s => s.sourceLocation.type === 'tableau');
    ...
    for (let i = 0; i < allSprites.length; i++) {
      ...
      this.tweens.add({
        ...
        onComplete: () => {
          sweepCompleted++;
          if (sweepCompleted === allSprites.length) {
            this.redealPhase2();
          }
        }
      });
  ...
  private redealPhase2(): void {
    // Destroy old sprites for tableau cards
    const state = this.engine.getState();
    const tableauCardIds = new Set<string>();
    ...
    for (const [id, sprite] of this.cardSprites) {
      if (tableauCardIds.has(id)) {
        sprite.destroy();
        this.cardSprites.delete(id);
      }
    }

    // Re-create and animate dealing
    const numPiles = state.tableau.length;
    const w = this.scale.width;
    let dealIndex = 0;
    let lastDealDelay = 0;
    ...
    const totalDuration = lastDealDelay + 200 + 50;
    this.time.delayedCall(totalDuration, () => {
      this.isDealAnimating = false;
      this.invalidateOverlapCache();
      this.createRedealButton(); // update button to show merci if applicable
      if (this.pendingResize && this.scaleResizeHandler) {
        this.pendingResize = false;
        this.scaleResizeHandler(this.scale.gameSize);
      }
  ...
  private executeMoveAndAnimate(from: LaBelleLucieLocation, to: LaBelleLucieLocation): void {
    try {
    ...
    for (let rank = 13; rank >= 1; rank--) {
      for (let fi = 0; fi < NUM_FOUNDATIONS; fi++) {
        ...
        if (pile && pile.length >= rank) {
          cardQueue.push({ card: pile[rank - 1], foundationIdx: fi });
        }
      }
    }
    ...
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        for (let i = 0; i < numPiles; i++) {
          const pileCards = state.tableau[i];
          if (pileCards.length === 0) continue;
          const card = pileCards[pileCards.length - 1];
          const from: LaBelleLucieLocation = { type: 'tableau', index: i };
          const to: LaBelleLucieLocation = { type: 'foundation', suit: card.suit };
  ...
  private flashHintCards(hint: LaBelleLucieMove): void {
  ...
  private getPileAtPoint(x: number, y: number): number {
    ...
    const numPiles = this.getNumPiles();
    for (let pile = 0; pile < numPiles; pile++) {
      const { row, col } = this.getPileRowCol(pile);
      const baseY = this.getPileBaseY(row);
      ...
      // Check if y is within this row's area
      const pileCards = this.engine.getState().tableau[pile];
      const overlap = this.getCurrentOverlap();
      const pileHeight = pileCards.length > 0
        ? this.cardHeight + (pileCards.length - 1) * overlap
        : this.cardHeight;
      if (y >= baseY - 5 && y <= baseY + pileHeight + 5) {
        return pile;
      }
    }
    return -1;
  ...
  private tryStartDrag(x: number, y: number): void {
    if (this.isMerciMode) return; // No drag in merci mode

    const pile = this.getPileAtPoint(x, y);
    if (pile !== -1) {
      const pileCards = this.engine.getState().tableau[pile];
      if (pileCards.length === 0) return;
      const cardIdx = this.getPilePickupIndexAtPoint(pile, x, y);
      if (cardIdx === -1) return;

      const sprite = this.cardSprites.get(pileCards[cardIdx].id);
      if (sprite) {
        ...
        this.activeDragFrom = { type: 'tableau', index: pile };
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    if (fIdx !== -1) {
      const suit = this.FOUNDATION_SUITS[fIdx];
      const to: LaBelleLucieLocation = { type: 'foundation', suit };
    ...
    const pile = this.getPileAtPoint(x, y);
    if (pile !== -1) {
      const to: LaBelleLucieLocation = { type: 'tableau', index: pile };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
        const tgtRow = this.engine.getState().tableau[pile].length;
        this.settleTargets = [this.getPileCardPosition(pile, tgtRow)];
        this.lastDragPointerPosition = null;
        this.animateDraggedCardsToTargets(this.settleTargets, () => {
          this.isSettlingDrag = false;
          if (this.pendingSettledMove) {
    ...
    if (this.activeDragFrom.type === 'tableau') {
      ...
      const fromPile = this.engine.getState().tableau[tableauFrom.index];
      const snapRow = Math.max(0, fromPile.length - 1);
      this.settleTargets = [this.getPileCardPosition(tableauFrom.index, snapRow)];
    } else {
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    // Handle merci mode clicks
    if (this.isMerciMode) {
      this.handleMerciCardClick(x, y);
      return;
    }
    ...
    let tappedFrom: LaBelleLucieLocation | null = null;
      ...
      if (pileCards.length > 0) {
        ...
        if (topCard.isFaceUp) {
          ...
          if (sprite && this.isPointWithinCardBounds(x, y, sprite.x, sprite.y, this.cardWidth, this.cardHeight)) {
            ...
            tappedFrom = { type: 'tableau', index: pile };
          }
        }
      }
    }
    ...
    if (tappedCardId && tappedFrom && tappedCardId === this.lastTapCardId && now - this.lastTapTime < 400) {
      const pileCards = state.tableau[(tappedFrom as { type: 'tableau'; index: number }).index];
      const card = pileCards.length > 0 ? pileCards[pileCards.length - 1] : null;
  ...
  private setupMouseDrag(): void {
    ...
    this.trackDomListener(canvas, 'mouseup', ((e: MouseEvent) => {
      ...
      if (this.isDragging) {
        ...
        // Handle click when not dragging (e.g. merci mode)
        this.handleTapOrDoubleTap(x, y);
      }
    }) as EventListener);
  }
  ...
  private setupTouchInput(): void {
    ...
    this.trackDomListener(canvas, 'touchend', ((e: TouchEvent) => {
      ...
      if (this.isDragging) {
        ...
        // Handle tap when not dragging (e.g. merci mode)
        const rect = canvas.getBoundingClientRect();
        const x = (e.changedTouches[0].clientX - rect.left) * (canvas.width / rect.width);
        const y = (e.changedTouches[0].clientY - rect.top) * (canvas.height / rect.height);
        this.handleTapOrDoubleTap(x, y);
