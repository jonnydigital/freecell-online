...
import { PenguinEngine, PenguinLocation, PenguinMove } from '../engine/PenguinEngine';
import { dealPenguinGame } from '../engine/Deck';
import { Card, Suit, Rank, SUIT_SYMBOLS, RANK_NAMES } from '../engine/Card';
...
interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: PenguinLocation;
}

export class PenguinScene extends Phaser.Scene {
  private engine!: PenguinEngine;
  ...
  private activeDragFrom: PenguinLocation | null = null;
  ...
  private pendingSettledMove: { from: PenguinLocation; to: PenguinLocation } | null = null;
  ...
  // Base rank for display
  private baseRank: Rank = 1;
  ...
  create(): void {
    ...
    const deal = dealPenguinGame(this.gameNumber);
    this.baseRank = deal.beakRank;
    this.engine = new PenguinEngine(this.gameNumber, deal.tableau, deal.flipper, deal.foundationCards, deal.beakRank);
  ...
  private getCascadeCardY(_col: number, row: number): number {
    const overlap = this.getCurrentOverlap();
    const topRow = this.boardOffsetY + this.topRowHeight + this.cascadeGap;
    return topRow + row * overlap;
  }
  ...
  // Flipper at column 0 top row
  private getFlipperPosition(): { x: number; y: number } {
    return { x: this.getColumnX(0), y: this.boardOffsetY };
  }

  // Foundations at columns 3,4,5,6 top row
  ...
  private drawSlots(): void {
    ...
    // Flipper slot (column 0, top row) with penguin label
    const flipperPos = this.getFlipperPosition();
    this.createSlot(flipperPos.x, flipperPos.y, '\uD83D\uDC27');

    // 4 Foundation slots (columns 3-6) with base rank label
    const baseLabel = RANK_NAMES[this.baseRank];
    const suitSymbols = ['\u2660', '\u2665', '\u2666', '\u2663'];
    for (let i = 0; i < NUM_FOUNDATIONS; i++) {
      const pos = this.getFoundationPosition(i);
      this.createSlot(pos.x, pos.y, baseLabel + suitSymbols[i]);
    }
  }
  ...
  private repositionAllCards(): void {
    ...
    // Flipper card
    if (state.flipper) {
      const sprite = this.cardSprites.get(state.flipper.id);
      if (sprite) {
        const pos = this.getFlipperPosition();
        sprite.x = pos.x; sprite.y = pos.y;
        sprite.sourceLocation = { type: 'flipper' };
        sprite.setDepth(55);
      }
    }
  }
  ...
  private dealCards(staggered: boolean = false): void {
    ...
    // Phase 1: Beak reveal — show base rank text briefly
    if (staggered) {
      const beakLabel = RANK_NAMES[this.baseRank];
      const beakText = this.add.text(w / 2, this.scale.height / 2, `Beak: ${beakLabel}`, {
      ...
      beakText.setOrigin(0.5);
      beakText.setDepth(6000);
      beakText.setAlpha(0);
      ...
      this.tweens.add({
        ...
        hold: 400,
        ease: 'Cubic.easeOut',
        onComplete: () => beakText.destroy(),
      });
    }

    // Phase 2: Animate beak cards flying to foundations
    const beakDelay = staggered ? 500 : 0;
    for (let i = 0; i < NUM_FOUNDATIONS; i++) {
      const suit = this.FOUNDATION_SUITS[i];
      const pile = state.foundations.get(suit)!;
      ...
      if (staggered) {
        ...
        const delay = beakDelay + dealIndex * 100;
        lastDealDelay = delay;
        this.tweens.add({
          targets: sprite,
          x: pos.x,
    ...
    // Phase 3: Deal flipper card
    if (state.flipper) {
      const flipperPos = this.getFlipperPosition();
      if (staggered) {
        const sprite = this.createCardSprite(state.flipper, w / 2 - this.cardWidth / 2, -this.cardHeight);
        sprite.sourceLocation = { type: 'flipper' };
        sprite.setDepth(500 + dealIndex);
        sprite.setScale(0.94);
        sprite.alpha = 0;
        ...
        const sprite = this.createCardSprite(state.flipper, flipperPos.x, flipperPos.y);
        sprite.sourceLocation = { type: 'flipper' };
        sprite.setDepth(55);
      }
    }

    // Phase 4: Deal cascade cards (7 columns)
    const cascadeStartDelay = staggered ? beakDelay + dealIndex * 100 + 150 : 0;
    const tableauDealStart = dealIndex;
    for (let col = 0; col < NUM_COLS; col++) {
      const cascade = state.tableau[col];
      for (let row = 0; row < cascade.length; row++) {
        ...
        if (staggered) {
          ...
          const delay = cascadeStartDelay + (dealIndex - tableauDealStart) * 18;
          lastDealDelay = delay;
          this.tweens.add({
            targets: sprite,
            x: pos.x,
  ...
  private executeMoveAndAnimate(from: PenguinLocation, to: PenguinLocation): void {
    try {
  ...
  private autoFinish(): void {
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        // Try moving cards from tableau/flipper to foundations
        let moved = false;
        const state = this.engine.getState();
        ...
        if (state.flipper) {
          ...
          if (this.engine.canMoveToFoundation(card, card.suit)) {
            this.engine.executeMove({ type: 'flipper' }, { type: 'foundation', suit: card.suit });
            moved = true;
          }
        }
        ...
        if (!moved) {
          for (let i = 0; i < NUM_COLS; i++) {
            const cascade = state.tableau[i];
            if (cascade.length === 0) continue;
            const card = cascade[cascade.length - 1];
            const from: PenguinLocation = { type: 'tableau', index: i };
            const to: PenguinLocation = { type: 'foundation', suit: card.suit };
  ...
  private flashHintCards(hint: PenguinMove): void {
  ...
  private isFlipperAtPoint(x: number, y: number): boolean {
    const pos = this.getFlipperPosition();
    return (
      x >= pos.x - 5 && x <= pos.x + this.cardWidth + 5 &&
      y >= pos.y - 5 && y <= pos.y + this.cardHeight + 5
    );
  }

  /**
   * Find the deepest card in a cascade that the player clicked on
   * and that starts a valid movable sequence to the bottom.
   */
  private getCascadePickupIndexAtPoint(col: number, x: number, y: number): number {
    ...
    // Check from bottom (top visually) to top (bottom visually)
    for (let i = cascade.length - 1; i >= 0; i--) {
      ...
      // For the bottom card, use full card height; for others, use overlap
      const hitHeight = i === cascade.length - 1 ? this.cardHeight : overlap;
      if (this.isPointWithinCardBounds(x, y, sprite.x, sprite.y, this.cardWidth, hitHeight)) {
        // Check if this starts a valid sequence
        const seq = this.engine.getMovableSequence(col, i);
        if (seq) return i;
      }
    }
  ...
  private tryStartDrag(x: number, y: number): void {
    // Check flipper
    if (this.isFlipperAtPoint(x, y)) {
      const flipper = this.engine.getState().flipper;
      if (flipper) {
        const sprite = this.cardSprites.get(flipper.id);
        if (sprite) {
          ...
          this.activeDragFrom = { type: 'flipper' };
    ...
    if (col !== -1) {
      ...
      const cardIdx = this.getCascadePickupIndexAtPoint(col, x, y);
      if (cardIdx === -1) return;

      const seq = this.engine.getMovableSequence(col, cardIdx);
      if (!seq) return;
      ...
      if (sprites.length > 0) {
        this.activeDragCards = sprites;
        this.activeDragVelocities = sprites.map(() => ({ x: 0, y: 0 }));

        for (let i = 0; i < sprites.length; i++) {
          sprites[i].setDepth(5000 + i);
          this.tweens.killTweensOf(sprites[i]);
          sprites[i].setScale(1.04);
        }

        this.activeDragFrom = { type: 'tableau', index: col, cardIndex: cardIdx };
        this.activeDragOffsets = { x: x - sprites[0].x, y: y - sprites[0].y };
        this.activeDragTarget = { x, y };
        this.lastDragPointerPosition = { x, y };
        this.isDragging = true;
        soundManager.cardSelect();
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    if (this.activeDragCards.length === 1) {
      ...
      if (fIdx !== -1) {
        ...
        const to: PenguinLocation = { type: 'foundation', suit };
      ...
      // Check flipper drop
      if (this.isFlipperAtPoint(x, y)) {
        const to: PenguinLocation = { type: 'flipper' };
        if (this.engine.isLegalMove(this.activeDragFrom, to)) {
          this.pendingSettledMove = { from: this.activeDragFrom, to };
          this.isDragging = false;
          this.isSettlingDrag = true;
    ...
    const col = this.getCascadeColumnAtPoint(x, y);
    if (col !== -1) {
      const to: PenguinLocation = { type: 'tableau', index: col };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
    ...
    // Also check if dropped on the top row near a cascade column
    // (allow drop on cascade even if in top row zone, as long as y is close)
    for (let c = 0; c < NUM_COLS; c++) {
      const cx = this.getColumnX(c);
      if (x >= cx - 5 && x <= cx + this.cardWidth + 5) {
        const to: PenguinLocation = { type: 'tableau', index: c };
        if (this.engine.isLegalMove(this.activeDragFrom, to)) {
          this.pendingSettledMove = { from: this.activeDragFrom, to };
          this.isDragging = false;
          this.isSettlingDrag = true;
          const tgtRow = this.engine.getState().tableau[c].length;
          this.settleTargets = this.activeDragCards.map((_, i) =>
            this.getCascadeCardPosition(c, tgtRow + i)
          );
          this.lastDragPointerPosition = null;
          this.animateDraggedCardsToTargets(this.settleTargets, () => {
            this.isSettlingDrag = false;
    ...
    if (this.activeDragFrom.type === 'tableau') {
      ...
      const fromCascade = this.engine.getState().tableau[tableauFrom.index];
      const startRow = tableauFrom.cardIndex ?? Math.max(0, fromCascade.length - 1);
      this.settleTargets = this.activeDragCards.map((_, i) =>
        this.getCascadeCardPosition(tableauFrom.index, startRow + i)
      );
    } else if (this.activeDragFrom.type === 'flipper') {
      const pos = this.getFlipperPosition();
      this.settleTargets = [{ x: pos.x, y: pos.y }];
    } else {
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    ...
    let tappedFrom: PenguinLocation | null = null;

    // Check flipper
    if (this.isFlipperAtPoint(x, y) && state.flipper) {
      const sprite = this.cardSprites.get(state.flipper.id);
      if (sprite && this.isPointWithinCardBounds(x, y, sprite.x, sprite.y, this.cardWidth, this.cardHeight)) {
        tappedCardId = state.flipper.id;
        tappedFrom = { type: 'flipper' };
      }
    }
    ...
    if (tappedCardId && tappedFrom && tappedCardId === this.lastTapCardId && now - this.lastTapTime < 400) {
      ...
      if (tappedFrom.type === 'flipper') {
        card = state.flipper;
      } else if (tappedFrom.type === 'tableau') {
        const cascade = state.tableau[(tappedFrom as { type: 'tableau'; index: number }).index];
        card = cascade.length > 0 ? cascade[cascade.length - 1] : null;
      }
