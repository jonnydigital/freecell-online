...
import { CruelScene } from './CruelScene';
import { ClockScene } from './ClockScene';
import { AccordionScene } from './AccordionScene';
import { LaBelleLucieScene } from './LaBelleLucieScene';
...
export function createPhaserConfig(
  parent: HTMLElement,
  variant: 'freecell' | 'bakers-game' | 'eight-off' | 'easy-freecell' | 'freecell-1cell' | 'freecell-2cell' | 'freecell-3cell' | 'spider' | 'klondike' | 'pyramid' | 'tripeaks' | 'golf' | 'yukon' | 'canfield' | 'forty-thieves' | 'scorpion' | 'seahaven' | 'beleaguered-castle' | 'penguin' | 'cruel' | 'clock' | 'accordion' | 'la-belle-lucie' | 'bisley' | 'aces-up' = 'freecell'
