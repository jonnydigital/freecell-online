...
export function createPhaserConfig(
  parent: HTMLElement,
  variant: 'freecell' | 'bakers-game' | 'eight-off' | 'easy-freecell' | 'freecell-1cell' | 'freecell-2cell' | 'freecell-3cell' | 'spider' | 'klondike' | 'pyramid' | 'tripeaks' | 'golf' | 'yukon' | 'canfield' = 'freecell'
  ...
  const initialScene = variant === 'canfield' ? CanfieldScene : variant === 'yukon' ? YukonScene : variant === 'golf' ? GolfScene : variant === 'tripeaks' ? TriPeaksScene : variant === 'pyramid' ? PyramidScene : variant === 'klondike' ? KlondikeScene : variant === 'spider' ? SpiderScene : FreeCellScene;
