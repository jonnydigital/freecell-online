...
interface CardSprite extends Phaser.GameObjects.Container {
  cardData: Card;
  sourceLocation: SeahavenLocation;
}

export class SeahavenScene extends Phaser.Scene {
  private engine!: SeahavenEngine;
  ...
  // Drag — single card only for Seahaven
  private activeDragCards: CardSprite[] = [];
  private activeDragFrom: SeahavenLocation | null = null;
  ...
  private pendingSettledMove: { from: SeahavenLocation; to: SeahavenLocation } | null = null;
  ...
  create(): void {
    ...
    const { tableau, freeCellCards } = dealSeahavenGame(this.gameNumber);
    this.engine = new SeahavenEngine(this.gameNumber, tableau, freeCellCards);
  ...
  private calculateLayout(): void {
    ...
    // 10 columns — same as Forty Thieves
  ...
  // Top row layout: [FC0] [FC1] [FC2] [FC3] gap [F-S] [F-H] [F-D] [F-C] gap gap
  // Using columns 0-3 for free cells, 6-9 for foundations
  ...
  private getFoundationPosition(index: number): { x: number; y: number } {
    return { x: this.getColumnX(6 + index), y: this.boardOffsetY };
  }
  ...
  private dealCards(staggered: boolean = false): void {
    ...
    // Deal cascade cards (10 columns, 5 cards each)
    for (let col = 0; col < NUM_COLS; col++) {
      const cascade = state.tableau[col];
      for (let row = 0; row < cascade.length; row++) {
    ...
    // Deal free cell cards (2 cards that start in free cells)
    for (let i = 0; i < NUM_FREECELLS; i++) {
      const card = state.freeCells[i];
      if (!card) continue;
  ...
  /**
   * Seahaven: only the top (last) card of a cascade can be picked up.
   */
  ...
  private executeMoveAndAnimate(from: SeahavenLocation, to: SeahavenLocation): void {
    try {
  ...
  private winCelebration(): void {
    ...
    // Collect cards from all 4 foundations
    const state = this.engine.getState();
  ...
  private autoFinish(): void {
    const doStep = () => {
      ...
      if (autoMoves.length > 0) {
        ...
        // Try moving cards from free cells / tableau to foundations
        let moved = false;
        const state = this.engine.getState();
        ...
        for (let i = 0; i < NUM_FREECELLS; i++) {
          ...
          const to: SeahavenLocation = { type: 'foundation', suit: card.suit };
  ...
  private flashHintCards(hint: SeahavenMove): void {
  ...
  private getFreeCellIndexAtPoint(x: number, y: number): number {
    for (let i = 0; i < NUM_FREECELLS; i++) {
      const pos = this.getFreeCellPosition(i);
  ...
  private tryStartDrag(x: number, y: number): void {
    // Check free cell card drag
    const fcIdx = this.getFreeCellIndexAtPoint(x, y);
    if (fcIdx !== -1) {
      const state = this.engine.getState();
      ...
      if (sprite) {
        ...
        this.activeDragFrom = { type: 'freecell', index: fcIdx };
  ...
  private tryDropDrag(x: number, y: number): void {
    ...
    if (fIdx !== -1) {
      ...
      const to: SeahavenLocation = { type: 'foundation', suit };
    ...
    // Check free cell drop
    const fcIdx = this.getFreeCellIndexAtPoint(x, y);
    if (fcIdx !== -1) {
      const to: SeahavenLocation = { type: 'freecell', index: fcIdx };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
        const pos = this.getFreeCellPosition(fcIdx);
        this.settleTargets = [{ x: pos.x, y: pos.y }];
        this.lastDragPointerPosition = null;
        this.animateDraggedCardsToTargets(this.settleTargets, () => {
          this.isSettlingDrag = false;
    ...
    const col = this.getCascadeColumnAtPoint(x, y);
    if (col !== -1) {
      const to: SeahavenLocation = { type: 'tableau', index: col };
      if (this.engine.isLegalMove(this.activeDragFrom, to)) {
        this.pendingSettledMove = { from: this.activeDragFrom, to };
        this.isDragging = false;
        this.isSettlingDrag = true;
    ...
    } else if (this.activeDragFrom.type === 'freecell') {
      const fcFrom = this.activeDragFrom as { type: 'freecell'; index: number };
      const pos = this.getFreeCellPosition(fcFrom.index);
      this.settleTargets = [{ x: pos.x, y: pos.y }];
    } else {
  ...
  private handleTapOrDoubleTap(x: number, y: number): void {
    ...
    let tappedFrom: SeahavenLocation | null = null;

    // Check free cells
    const fcIdx = this.getFreeCellIndexAtPoint(x, y);
    if (fcIdx !== -1 && state.freeCells[fcIdx]) {
      tappedCardId = state.freeCells[fcIdx]!.id;
      tappedFrom = { type: 'freecell', index: fcIdx };
    }
    ...
    if (tappedCardId && tappedFrom && tappedCardId === this.lastTapCardId && now - this.lastTapTime < 400) {
      ...
      if (tappedFrom.type === 'freecell') {
        card = state.freeCells[(tappedFrom as { type: 'freecell'; index: number }).index];
      ...
      if (card) {
        ...
        // Try to send to a free cell as fallback
        for (let i = 0; i < NUM_FREECELLS; i++) {
          if (state.freeCells[i] === null) {
            const fcTo: SeahavenLocation = { type: 'freecell', index: i };
            if (this.engine.isLegalMove(tappedFrom, fcTo)) {
              this.executeMoveAndAnimate(tappedFrom, fcTo);
              this.lastTapCardId = null;
              this.lastTapTime = 0;
              return;
            }
